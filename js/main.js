 ...
 */
define(["dojo/_base/declare", "dojo/_base/Color", "dojo/parser", "dojo/has", "dojo/query", "dijit/registry", "dojo/window", "dojo/promise/all", "dojo/Deferred", "dojo/_base/lang", "esri/arcgis/utils", "dojo/dom", "dojo/dom-attr", "dojo/dom-construct", "dojo/dom-style", "dojo/dom-class", "dojo/on", "esri/dijit/Legend", "esri/dijit/HomeButton", "esri/lang", "dijit/layout/ContentPane", "dojox/layout/ExpandoPane", "dijit/form/Select", "application/grid", "application/clickLegend", "dojo/domReady!"], function (
declare, Color, parser, has, query, registry, win, all, Deffered, lang, arcgisUtils, dom, domAttr, domConstruct, domStyle, domClass, on, Legend, HomeButton, esriLang, ContentPane, ExpandoPane, Select, dataGrid, ClickLegend) {
    //var reportGrid;
    return declare(null, {
        config: {},
        mapInfo: [],
        ...
        startup: function (config) {
            ...
            if (config) {
                ...
                if(this.config.showTitleAndDescription){
                    var content = esriLang.substitute(this.config, "<div id='reports'></div>");
            ...
            all(promiseList).then(lang.hitch(this, function (results) {
                for (i = 0; i < results.length; i++) {
                    if (results[i] && results[i].map) {
                        ...
                        var map = result.map;
                        map.krig = map.getLayer(map.layerIds[1]);
                        map.krig.setVisibility(false);
                        map.fLayer = map.getLayer(map.layerIds[2]);
                        ...
                        var map_title = domConstruct.create("div",{
                            id: result.map.id + "_title",
                            "class": "mapTitle bg fg",
                            innerHTML: "<span id='" + result.map.id + "_year'></span>" + "<span id='" + result.map.id + "_t'>(T" + (i - 1) + ")</span>"//registry.byId("reportYearSelect").get("displayedValue")//result.itemInfo.item.title
                        },result.map.id + "_root");
                        ...
                        if (i === 2) {
                            ...
                            var forward = domConstruct.create("div", {
                                "id": "forward",
                                "title": "Advance year",
                                "class": "icon-chg fg icon-forward"
                            }, result.map.id);

                            var back = domConstruct.create("div", {
                                "id": "back",
                                "title": "Back one year",
                                "class": "icon-chg fg icon-back"
                            }, result.map.id);
                ...
                //update theme
                this._updateTheme();
                var grid = new dataGrid(this.mapInfo);
                //this._updateLayers();
            }));
        ...
        _createCell: function (count, cols, row, i) {
            ...
            var cell = domConstruct.create("div", {
                ...
                "id": "map_" + i,
                "style": "height: 100%; border-color: black"// + win.getBox().h / Math.ceil(this.config.webmaps.length / cols) * 0.75 + "px;"
            }, row);
            return cell;
        },
        _createRow: function () {
        ...
        _createInfoContent: function (details, layer) {
            ...
            domConstruct.create("div",{
                ...
                innerHTML: "<b>Basemap:</b><div id='basemapPane'></div><br><b>Layers:</b><div id='featurePane'></div>"
            },layer.map.id + "_root");

            var mapItems = this.mapInfo;

            var baseSelect = new Select({
                id: "basemapSelect",
                labelAttr: "label",
                options: [
                    { value: "gray", label: "Light Gray" },
                    { value: "topo", label: "ESRI Topographic" },
                    { value: "satellite", label: "Imagery" },
                    { value: "hybrid", label: "Imagery w/Labels" },
                    { value: "streets", label: "ESRI Street Map" },
                    { value: "terrain", label: "Terrain" }
                ],
                onChange: function(){
                    for (var i = 0; i < mapItems.length; i++) {
                        mapItems[i].map.setBasemap(this.get("value"));
                    }
                }
            });
            baseSelect.placeAt("basemapPane");

            var legend = new ClickLegend();
            legend.mapItems = mapItems;
            //alert(map.fLayer.url);
            legend.setLayers(map.fLayer.url);
